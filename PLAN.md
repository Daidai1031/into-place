...
- [x] `mode:auto` 抠形已全部完成(云端跑通:npm `@rmbg/model-silueta` 拿到 md5 一致的官方权重,见 `scripts/README.md`)。**分流结论(实测)**:照片/半调印刷抠形干净可用(asset_001/012/014/015/017/018);线刻版画不可用(asset_006 输出晕影,已删),版画一律纸卡
- 完成标准:✅ 22 个图层就位(14 纸卡 + 1 地图 + 7 抠形建筑/物体),够搭全部 5 个场景。
