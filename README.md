...
Into Place 不是在流程末尾调用一次视频模型，而是把 fal 作为贯穿研究、叙事、分镜和镜头生成的多模型创作层。每个阶段选择不同能力，并把上一步经过用户审核的结构化结果交给下一步。
...
| Collage composition | `nano-banana/edit` + `any-llm/vision` | 先生成构图参考，再由 vision LLM 转成真实 cutout 的布局 JSON | ✅ 已接通 |
| Generated storyboard | nano-banana 2 / FLUX.2 | 从 beat、film premise 与档案 references 生成 16:9 frame，并支持语言编辑与拖入素材 | ✅ 已接通 |
| Video shot | Kling / Happy Horse / Veo 候选 | 将用户审核后的 storyboard frame 作为首帧，生成逐镜 I2V 片段 | 🚧 Queue 接入中 |

### 1. fal 多模型路由，而不是单一黑盒

所有模型集中注册在 `lib/models.ts`，所有 fal 请求都封装在 server-only 的 `lib/fal-server.ts`。Story、vision、image edit 与 I2V 使用同一套地点、beat、reference asset IDs 和审核状态，避免每一步重新输入一段无法追踪的自由 prompt。
...
- **Frame control：** 每个 beat 可以反复生成分镜、选择 reference set、用自然语言编辑，或把某张档案 cutout 拖到指定位置。
...
这让模型生成的重点落在运动与转场，而不是随意重写人物、建筑和档案文字。
...
- 没有 `FAL_KEY` 或调用失败时，系统返回明确标注的模拟 frame，并允许切换 manual collage；演示流程不会把 fallback 冒充真实生成。
