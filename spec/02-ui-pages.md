...
- Assembly applies a unified grade to all shots (muted blue-gray / sepia, brighter with lifted blacks) and fades the ending to black. See “Assembly post-production” in `spec/04-shot-router.md`; the assembled master is silent until the optional audio pass.
- Player + mode description (local preview / simulation); `+ Save to library`; local users with a FAL key also see equal-level `Background music` and `Narration` switches. Either or both may be selected before the paid confirmation. Narration is drafted from the current Story map, voiced through fal Kokoro, shown in the UI for review, and mixed with any selected Lyria score before the player refreshes.
