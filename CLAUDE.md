...
- **合成后期在 `lib/film-assemble.ts` 的 `xfadeConcat` 里做**(`/api/assemble` 与 `scripts/render-film.mts` 共用):拼接前给每个镜头套同一份 `GRADE`(muted blue-gray 阴影 + 暖 sepia 中间调,提亮抬黑位对抗 i2v 偏暗,`saturation 0.85`,护高光),让分别生成的镜头统一;片尾 `fadeOutDur`(默认 1.0s)渐隐到黑。`GRADE` 是单一可调常量,偏暗/偏灰在此微调。i2v 生成仍 `generate_audio:false`(视频一律静音),音轨在**下面的独立音频 pass** 里加,保持可分离、可重混。
- **音频 pass = 导出前最后一步(2026-07-19 落地)**:`scripts/add-audio.mts` 给静音成片加(1)配合画面的 diegetic foley + (2)一条克制的配乐,再 FFmpeg 混音 mux 进 `final/<slug>.mp4`。全走 fal:foley 用 **MMAudio V2**(`fal-ai/mmaudio-v2`,$0.001/s,video→audio,吃 `video_url`+`prompt`+`negative_prompt`+`duration`,返回带音频的 video),配乐用 **Lyria 2**(`fal-ai/lyria2`,$0.10/30s,text→music,无 duration 参数→后期 loop/trim),二者已 fal MCP 核价(见 `lib/models.ts` `AudioModel`/`MMAUDIO_V2`/`LYRIA2`)。声音**声明式**:cue 写在 `data/presets/<slug>.json` 每个 beat 的 `sound{diegetic[],ambient}`(RI 五幕已填:写字沙沙/翻页/盖章、脚步/关门、倒水/长凳/笔尖、河水波浪/印刷机/演说者起伏声/人群议论、键盘/孩童嬉戏),由 `lib/prompt-compiler.ts` 的 `compileSoundPrompt`/`compileWholeFilmSoundPrompt`/`compileMusicPrompt` 编译(**不手写 prompt**,复用 STYLE 情绪);`SOUND_NEGATIVE` 把配乐/清晰人声/字幕/现代噪音排除在 foley 之外——人群与演说只作**含混不可辨的环境声**,守住"no dialogue/subtitles"红线;`MUSIC_NEGATIVE` 排除 horror/鼓点/人声等。
  - 两种源模式自动判定:`renders/<slug>/clip_XX.mp4` 存在(真 render-film 跑过)→ **per-clip**,逐镜生成 foley 并按 `clipOffsets()`(与 `xfadeConcat` 同一 offset 公式)放到该镜在成片时间轴上的起点;只有 `final/<slug>.mp4`(如当前 demo 12.67s)→ **whole-cut**,一条合并 prompt 跑整片。混音在 `lib/film-assemble.ts` `muxFilmAudio()`:每条 foley `adelay` 到起点、配乐 `-stream_loop` 铺满再 duck(`musicGainDb` 默认 -13dB),全轨强制 stereo/48k(MMAudio 输出是 mono,不强制会把配乐塌成单声道)、随画面 `afade` 收尾、`alimiter` 防削顶,视频 `-c:v copy` 不重编码。
  - 幂等 & 省钱:首次把静音成片存 `renders/<slug>/silent-master.mp4` 作耐久静音源,之后每次 mux 覆盖 `final`;`--remix`(读 `data/scenes/generated/<slug>/audio-provenance.json` 复用已生成音频)只重混、**不再调 fal**,用来调 gain。付费需 `--yes`,>$5 需 `--confirm`。provenance 写 request_id/模型/prompt/参数/成本,不写密钥。跑完 `node scripts/sync-public.mjs`。当前 demo 已出音:whole-cut,foley $0.013 + music $0.10 = **$0.113**,`final`/`public/films` 均为 h264+aac stereo。
