...
- **默认故事预设**:`data/presets/<slug>.json`(Roosevelt Island = Direction One 五幕 + 每 beat references),`lib/presets.ts` 服务端加载,经 Story/Storyboard 页种子化;Story 有「use the built-in story」按钮,Storyboard 每 beat 默认 references 来自预设。
